...
public class ChordalGraph extends SimpleGraph<Integer, DefaultEdge> implements UndirectedGraph<Integer, DefaultEdge> {
	...
	public boolean isEdgeRemovable(Integer vertex1, Integer vertex2) {
		if (containsEdge(vertex1, vertex2) || containsEdge(vertex2, vertex1)) {
			...
			// keeping chordality requires the two vertices to be
			// part
			// of one clique only
	...
	/**
	 * To use with extreme caution, and only if the structure is not
	 * modified afterwards
	 * 
	 * @param vertex1
	 * @param vertex2
	...
	/**
	 * @return the size of the biggest clique for the graph associated to
	 *         this model
	 */
	...
	/**
	 * Not fully tested
	 * @return a junction tree of the chordal graph
	 */
	@Deprecated
	...
	public DefaultEdge addSecuredEdge(Integer a, Integer b, MyPriorityQueue pq, boolean verbose) {
		...
		ConnectivityInspector<Integer, DefaultEdge> inspectorgMinusSabBeforeAB = new ConnectivityInspector<Integer, DefaultEdge>(
						gMinusSabBeforeAB);
		...
		// considering every edge of the clique graph for removal (step
		// 2)
	...
	private void removeInvalidEdgesCG(BitSet Sab, Set<CliqueGraphEdge> cgEdgesBeforeNewNode,
		...
		for (CliqueGraphEdge cEdge : cgEdgesBeforeNewNode) {
			...
			if (S12.equals(Sab)) {
				...
				if (inspectorGpp.pathExists(vFromC1mS12, vFromC2mS12)) {
					// if(c1mS12.cardinality()>1 &&
					// c2mS12.cardinality()>1){
	...
	private void disableEdge(Integer a, Integer b, MyPriorityQueue pq, boolean verbose) {
		...
		if (pq != null)
			pq.disableEdge(a, b);
		if (verbose)
			System.out.println("\t(" + a + "," + b + ") not accessible any more:" + eligibleEdges[a][b]);
	}
	...
	private void enableEdge(Integer a, Integer b, CliqueGraphEdge edge, MyPriorityQueue pq, boolean verbose) {
		...
		if (verbose)
			System.out.println("\t(" + a + "," + b + ") now accessible:" + eligibleEdges[a][b]);
